...
public class Orientation implements SensorEventListener {
    ...
    public interface Listener {
        void onOrientationChanged(float pitch, float roll, int azimuth);
    }
    ...
    public void startListening(Listener listener) {
        ...
        if (mRotationSensor == null) {
            Log.w(Orientation.class.getSimpleName(),
    ...
    private void updateOrientation(float[] rotationVector) {
        ...
        int mAzimuth = (int) (Math.toDegrees(SensorManager.getOrientation(adjustedRotationMatrix, orientation)[0]) + 360) % 360;

        // Convert radians to degrees
        float pitch = orientation[1] * -57;
        float roll = orientation[2] * -57;

        mListener.onOrientationChanged(pitch, roll, mAzimuth);
    }
}